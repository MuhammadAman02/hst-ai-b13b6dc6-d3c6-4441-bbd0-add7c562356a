...
def check_dependency_compatibility():
    ...
    # Check if we're using Poetry for dependency management
    try:
        import poetry
        print("Using Poetry for dependency management. Version conflicts should be automatically resolved.")
    except ImportError:
        # Poetry not installed, add a recommendation
        if issues:
            issues.append("Consider using Poetry for more robust dependency management")
...
if compatibility_issues:
    ...
    # Check if dependency_resolver.py exists
    resolver_exists = os.path.exists("dependency_resolver.py")
    checker_exists = os.path.exists("compatibility_checker.py")
    
    # Check if Poetry is installed
    poetry_installed = verify_module_installed("poetry")
    if poetry_installed:
        print("\nYou can resolve dependency conflicts using Poetry:")
        if resolver_exists:
            print("1. Run: python dependency_resolver.py --diagnose  (to see detailed conflict information)")
            print("2. Run: python dependency_resolver.py --update    (to automatically update dependencies)")
        else:
            print("1. Edit pyproject.toml with the correct version constraints")
            print("2. Run: poetry update")
            print("3. Reinstall dependencies: poetry install")
    else:
        print("\nConsider using Poetry to manage dependency conflicts:")
        print("1. Install Poetry: pip install poetry")
        print("2. Run: python setup_poetry.py")
        print("3. Follow the instructions in README.md for dependency management with Poetry")
    
    if checker_exists and not poetry_installed:
        print("\nYou can also run the compatibility checker to identify specific issues:")
        print("  python compatibility_checker.py")
    
    print("\nPlease refer to the README.md for more information on dependency compatibility.")
    print("Poetry provides robust dependency resolution that can automatically resolve these conflicts.")
